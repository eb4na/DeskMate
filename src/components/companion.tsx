...
export function Companion({ pose, size = 'full' }: Props) {
  const { defaultCompanionId, equippedShopItems } = useApp();
  const config = POSE_CONFIG[pose];
  const isFull = size === 'full';
  const theme = useTheme();
  const activeTheme = getThemeEffect(equippedShopItems);
  const activeOutfit = getOutfitEffect(equippedShopItems);
  ...
  const labelText = [config.label, activeTheme?.label, activeDecoration?.label].filter(Boolean).join(' · ');
