...
const RECIPE_FOOD_ID: Record<string, string> = {
  recipe_rollcake: 'strawberry-shortcake',
...
/** Localized description for any shop item (shopDescs.<id> in the locale files).
 *  Falls back to the raw English `description` for anything unmapped. */
export function localizeShopItemDescription(
  item: { id: string; description?: string },
  t: (key: string, opts?: Record<string, unknown>) => string,
): string {
  return t(`shopDescs.${item.id}`, { defaultValue: item.description ?? '' });
}
